-- businesses.slug is NOT NULL on remote; create_team_account must populate it.

-- Backfill any rows missing slug (legacy / partial applies)
DO $$
BEGIN
  IF EXISTS (
    ...
    FROM (
      SELECT
        ...
        COALESCE(
          NULLIF(trim(a.slug), ''),
          kit.slugify(COALESCE(NULLIF(trim(b2.name), ''), 'business'))
            || '-'
            || substring(replace(b2.id::text, '-', ''), 1, 8)
        ) AS candidate
      FROM public.businesses b2
      LEFT JOIN public.accounts a ON a.id = b2.account_id
      WHERE b2.slug IS NULL
    ) sub
    WHERE b.id = sub.id;
  END IF;
END $$;

-- Replace create_team_account: set businesses.slug from workspace account slug
