...
/** One template: legacy headings plus level-gated optional catalogue fields. */
export function hubSectionsForLevel(
  level: SurveyLevel | number | string | null | undefined,
): HubSurveySection[] {
  const resolved = normalizeSurveyLevel(level);
  const visible = sectionsVisibleAtLevel(resolved);
  const legacyKeys = new Set(
    BUILDING_SURVEY_SECTIONS.map((section) => section.key),
  );

  const legacy = BUILDING_SURVEY_SECTIONS.map((section) => {
    const item = surveySectionByKey(section.key);
    return {
      key: section.key,
      heading: section.heading,
      group: section.group,
      letter: section.letter ?? item?.letter ?? '',
      optional: false,
    };
  });
