...
export function storagePathFromProjectPictureUrl(
...
function projectPhotoPath(accountId: string, projectId: string) {
  return `${accountId}/project-${projectId}`;
}

export async function storeProjectPhotoBytes(input: {
  accountId: string;
  projectId: string;
  ...
  const existingPath = storagePathFromProjectPictureUrl(
    input.existingPictureUrl,
  );
  const nextPath = projectPhotoPath(input.accountId, input.projectId);
  ...
  if (uploadError) {
    console.error('[projects] upload-photo:', uploadError.message);
