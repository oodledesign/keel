...
import { UpdateBusinessSeatQuantitySchema } from '../schema/business-seats.schema';

export const updateBusinessSeatQuantityAction = enhanceAction(
  async (data) => {
    ...
    const assigned = memberCount ?? 0;
    if (data.quantity < assigned) {
      throw new Error(
        `Cannot schedule fewer than ${assigned} seats while that many members are on the workspace. Remove members first.`,
      );
    }
    ...
    if (
      ...
      await client
        ...
        .update({
          ...
          max_project_guests: maxProjectGuestsForBillableSeats(result.quantity),
  ...
  {
    schema: UpdateBusinessSeatQuantitySchema,
  },
);
