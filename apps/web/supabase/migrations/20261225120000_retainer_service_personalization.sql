...
-- Additive only. Existing catalogue rows stay workspace-scoped.
-- Existing project_retainer_services allowlists become project overrides.
...
SET retainer_services_source = 'inherited'
WHERE retainer_services_source IS NULL OR retainer_services_source = '';
...
WHERE services_source IS NULL OR services_source = '';
...
-- Least destructive backfill: an existing allowlist is a project override.
UPDATE public.project_retainers pr
SET services_source = 'custom'
WHERE pr.services_source = 'inherited'
  AND EXISTS (
    SELECT 1
