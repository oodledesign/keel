...
**Subscription tiers** (`ozer-addon-campaigns`, monthly only — no yearly SKUs):
...
**Send packs** (one-off lasts 12 months; monthly replenishes each cycle; stackable):
...
Existing +10k / +50k send SKUs are reused at their current amounts (do not create cheaper duplicate +10k/+50k prices).

**Contact bumps** (monthly only; no send units; stackable):
