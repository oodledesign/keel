...
import {
  ExtensionExtractTasksSchema,
  extractTasksFromExtensionText,
} from '~/lib/extension/extract-tasks';
...
export async function POST(request: Request) {
  ...
  const parsed = ExtensionExtractTasksSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const result = await extractTasksFromExtensionText({
      userId: auth.user_id,
      body: parsed.data,
    });
