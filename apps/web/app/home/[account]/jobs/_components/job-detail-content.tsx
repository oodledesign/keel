  ...
  return (
    ...
    >
                ...
                <>
                  ...
                  {assignments.map((a) => {
                    ...
                    return (
                      <span
                        key={a.user_id}
                        className="inline-flex items-center gap-1 rounded-full border border-[color:var(--workspace-shell-border)] bg-[var(--workspace-control-surface)]/60 py-0.5 pr-2 pl-0.5"
                        title={
                          a.role_on_job ? `${a.role_on_job}: ${label}` : label
                        }
                      >
                        <ProfileAvatar
                          displayName={label}
      ...
      >
        ...
        <TabsContent
          value="project"
          className="mt-0 flex flex-1 flex-col p-4 md:p-5"
        >
          <JobProjectWorkspace
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
            <div className="prose prose-invert max-w-none text-sm">
              ...
              <ul className="mt-4 list-disc space-y-1 pl-5 text-[var(--workspace-shell-text-muted)]">
                ...
                <li>
                  Team: {assignments.length} member
                  ...
                  <>
                    <ul className="mt-3 space-y-3">
                      {notes.map((n) => (
                        <li
                          key={n.id}
                          className="border-l-2 border-[color:var(--workspace-shell-border)] pl-3 text-sm"
                        >
                          <p className="whitespace-pre-wrap text-[var(--workspace-shell-text-muted)]">
                            {n.note}
                          </p>
                    ...
                    {canEditJobs && (
                      <form onSubmit={handleAddNote} className="mt-4">
                        ...
                        <Button
                          type="submit"
                          size="sm"
                          className={`mt-2 ${newNote.trim() && !addingNote ? 'bg-[var(--ozer-accent)] text-[var(--ozer-white)] hover:bg-[var(--ozer-accent-hover)]' : 'bg-[var(--workspace-shell-panel-hover)] text-[var(--workspace-shell-text-muted)] hover:bg-[var(--workspace-shell-panel-hover)]'}`}
