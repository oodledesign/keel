...
import { getProposalPortalLink, sendProposal } from '../_lib/server/server-actions';
...
export function ProposalSendPanel({
  ...
  const [subject, setSubject] = useState(initialSubject ?? DEFAULT_PROPOSAL_EMAIL_SUBJECT);
  const [body, setBody] = useState(initialBody ?? DEFAULT_PROPOSAL_EMAIL_BODY);
  const [signature, setSignature] = useState(initialSignature ?? DEFAULT_PROPOSAL_EMAIL_SIGNATURE);
