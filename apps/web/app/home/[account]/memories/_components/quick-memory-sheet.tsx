  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex w-full flex-col gap-0 overflow-y-auto sm:max-w-md">
        ...
        {open ? (
          <QuickMemoryForm
            key={defaultChildIds.join(',')}
            accountId={accountId}
            accountSlug={accountSlug}
            people={people}
            defaultChildIds={defaultChildIds}
...
function QuickMemoryForm({
  ...
  const [childIds, setChildIds] = useState(defaultChildIds);
  ...
  function addFiles(list: FileList | null) {
    ...
    for (const file of Array.from(list)) {
      try {
        ...
        toast.error(
          getUnknownErrorMessage(error, 'Could not attach that file'),
        );
      }
    }
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        ...
        toast.error(getUnknownErrorMessage(error, 'Could not save memory'));
      }
    });
  }
