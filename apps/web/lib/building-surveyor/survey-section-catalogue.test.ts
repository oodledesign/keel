...
describe('survey section catalogue', () => {
  ...
  it('covers every Phase 1–2 section key so the hub can dual-write', () => {
    ...
    for (const section of BUILDING_SURVEY_SECTIONS) {
      expect(catalogueKeys.has(section.key)).toBe(true);
    }
  });

  it('exposes optional L2 / L3 detail fields on one hub template', () => {
    const l2 = hubSectionsForLevel(2);
    const l3 = hubSectionsForLevel(3);
    expect(l2.some((item) => item.key === 'windows')).toBe(true);
    expect(l3.some((item) => item.key === 'windows')).toBe(true);
    expect(l2.some((item) => item.key === 'energy_efficiency_rating')).toBe(
      true,
    );
    expect(l3.some((item) => item.key === 'energy_efficiency_rating')).toBe(
      true,
    );
    expect(l2.some((item) => item.key === 'energy')).toBe(false);
    expect(l3.some((item) => item.key === 'energy')).toBe(true);
    expect(l2.some((item) => item.key === 'valuation')).toBe(true);
    expect(l3.some((item) => item.key === 'valuation')).toBe(false);
    expect(l2.some((item) => item.key === 'energy_heating')).toBe(false);
    expect(l3.some((item) => item.key === 'energy_heating')).toBe(true);
  });
});
