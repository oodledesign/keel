...
import { mondayWeekStart } from '~/home/(user)/life/family/_lib/server/family-meal.dates';
import { authenticateNativeRequest } from '~/lib/native/auth';
import { handleNativeError } from '~/lib/native/http';
import { listNativeMealPlan, nativeMealScope } from '~/lib/native/meals';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = await listNativeMealPlan(
      auth.context.supabase,
    ...
    return handleNativeError(error, 'meal-plan');
  }
}
