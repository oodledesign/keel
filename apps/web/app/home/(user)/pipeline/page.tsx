...
function PipelineSkeleton() {
  return (
    <div className="flex min-h-full w-full flex-col gap-6 px-4 pt-6 pb-12 md:px-6 lg:px-8">
      <div className="h-10 w-48 animate-pulse rounded-xl bg-[var(--workspace-shell-sidebar-accent)]" />
      <div className="flex gap-4 overflow-x-auto">
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={i} className="flex min-w-[260px] flex-1 flex-col gap-3">
            <div className="h-6 w-24 animate-pulse rounded-lg bg-[var(--workspace-shell-sidebar-accent)]" />
            <div className="h-32 animate-pulse rounded-2xl bg-[var(--workspace-shell-sidebar-accent)]" />
            <div className="h-32 animate-pulse rounded-2xl bg-[var(--workspace-shell-sidebar-accent)]" />
          </div>
        ))}
      </div>
