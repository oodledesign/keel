...
export function isSenderIgnored(
  ...
  if (!from || !ignoredSenders?.length) {
    return false;
  }

  return ignoredSenders.some(
    (ignored) => extractEmailAddress(ignored) === from,
  );
}

/**
 * Ignore a sender for this mailbox: persist the address, mark matching threads
 * as no_reply, and dismiss pending suggested action items from them.
 */
export async function ignoreEmailSenderAndDismissSuggestions(
  ...
  const nextIgnored = normalizeIgnoredSenders([
    ...(((settings as { ignored_senders?: string[] | null } | null)
  ...
  const dismissedCount = await dismissSuggestedItemsFromSender(
    client,
    userId,
    connectionId,
    sender,
...
export async function removeIgnoredEmailSender(
  ...
  const normalized = extractEmailAddress(sender);
...
async function dismissSuggestedItemsFromSender(
  client: SupabaseClient,
  userId: string,
  connectionId: string,
  sender: string,
  ...
  if (threadIdsNeedingTip.length > 0) {
    ...
    for (const row of tipMessages ?? []) {
      ...
      tipByThread.set(threadId, (row.from_address as string | null) ?? null);
    }
  }
  ...
  for (const row of rows) {
    ...
    if (isSenderIgnored(from, [sender])) {
  ...
  if (matchingThreadIds.size > 0) {
    const { error: threadUpdateError } = await client
      ...
      .update({
        ...
        assistant_category_reason: `Sender ignored (${sender})`,
        updated_at: new Date().toISOString(),
      })
      .eq('user_id', userId)
