-- Referral program + content-reward credits (see schemas/41-rewards.sql)
...
create table if not exists public.balance_transactions_log (
  ...
  constraint balance_transactions_log_idempotency_unique unique (idempotency_key)
);
...
begin
  ...
  set
    referral_code = coalesce(
      user_settings.referral_code,
      kit.generate_referral_code ()
    )
  where user_settings.referral_code is null;
end;
$$;
...
begin
  ...
  perform kit.ensure_user_settings_with_referral (p_user_id);

  select us.referral_code into code
  from public.user_settings us
...
begin
  for r in
    select us.user_id
    from public.user_settings us
    where us.referral_code is null
  loop
    perform kit.ensure_user_settings_with_referral (r.user_id);
  end loop;
end;
$$;
...
-- Enforce content reward caps atomically on insert
create or replace function public.enforce_content_submission_cap ()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  if public.content_reward_would_exceed_caps (
    new.user_id,
    new.reward_amount_pence,
    2000,
    18000
  ) then
    raise exception 'Content reward cap exceeded';
  end if;
...
drop trigger if exists content_submissions_cap_check on public.content_submissions;

create trigger content_submissions_cap_check
before insert on public.content_submissions
for each row
execute function public.enforce_content_submission_cap ();
