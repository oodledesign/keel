...
function getSesConfig() {
  ...
  if (!region || !accessKeyId || !secretAccessKey || !from) {
    throw new Error(
      'Missing SES configuration. Set AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, and SES_FROM_EMAIL.',
    );
  }

  return { region, accessKeyId, secretAccessKey, from: sanitizeEmailSender(from) };
}
...
function buildRawEmail(params: {
  ...
  const headers = [
    ...
    'MIME-Version: 1.0',
    `X-Campaign-ID: ${sanitizeHeader(params.campaignId)}`,
