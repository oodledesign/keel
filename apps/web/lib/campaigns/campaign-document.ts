...
export const CAMPAIGN_LOGO_VARIANTS = [
  'primary',
  'on_light',
  'on_dark',
] as const;

export type CampaignLogoVariant = (typeof CAMPAIGN_LOGO_VARIANTS)[number];
...
export type CampaignBlockStyle = {
...
type WithBlockStyle<T> = T & CampaignBlockStyle;
...
export type CampaignBlock =
  | WithBlockStyle<{
      ...
      type: 'logo';
      align?: CampaignAlign;
      logoVariant?: CampaignLogoVariant;
    }>
  | WithBlockStyle<{
      id: string;
      type: 'heading';
  ...
  | WithBlockStyle<{ id: string; type: 'divider' }>
  | WithBlockStyle<{ id: string; type: 'spacer'; height: number }>
  | WithBlockStyle<{
      id: string;
      type: 'columns';
  ...
  | WithBlockStyle<{ id: string; type: 'footer'; text: string }>
  | WithBlockStyle<{ id: string; type: 'html'; html: string }>;
...
const LogoVariantSchema = z.enum(CAMPAIGN_LOGO_VARIANTS);
const ImageSizeSchema = z.enum(CAMPAIGN_IMAGE_SIZES);

export const CampaignPaddingSchema = z.object({
...
const CampaignBackgroundColorSchema = z.preprocess(
  (value) => {
...
const BlockStyleFields = {
  backgroundColor: CampaignBackgroundColorSchema,
  padding: CampaignPaddingSchema.optional(),
};
...
export const CampaignBlockSchema = z.discriminatedUnion('type', [
  z.object({
    ...
    logoVariant: LogoVariantSchema.optional(),
    ...BlockStyleFields,
  }),
  z.object({
    id: z.string().min(1).max(80),
  ...
  z.object({
    ...
    height: z.number().int().min(20).max(1200).optional(),
    ...BlockStyleFields,
  }),
  z.object({
    id: z.string().min(1).max(80),
...
export const CAMPAIGN_BLOCK_LIBRARY: Array<{
  ...
  { type: 'image', label: 'Image', description: 'Upload, library, or URL' },
