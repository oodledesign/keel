...
async function JoinTeamAccountPage(props: JoinTeamAccountPageProps) {
  ...
  // After accepting invite, redirect to onboarding (per-membership onboarding)
  const onboardingUrl = `${pathsConfig.app.onboarding}?account_id=${invitation.account.id}`;

  // Determine if we should show the account setup step (Step 2)
  // Invited users must explicitly choose their preferred email sign-in method
  // before entering the workspace for the first time.
