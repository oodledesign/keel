...
class ProjectPhasesService {
  ...
  private async verifyJob(accountId: string, jobId: string) {
    const { data, error } = await this.db
      .from('jobs')
      .select('id, account_id, title, client_id, status, priority, start_date, due_date, value_pence, cost_pence')
      .eq('id', jobId)
  ...
  async moveTask(input: MoveTaskInput) {
    ...
    if (task.job_id && task.job_id !== input.jobId) {
    ...
    const { data, error } = await this.db
      ...
      .eq('id', input.taskId)
      .select(
        'id, title, status, priority, due_date, sort_order, phase_id, job_id, user_id',
      )
      .single();
  ...
  async ensurePhasePage(input: EnsurePhasePageInput) {
    ...
    const { data: phase, error: phaseErr } = await this.db
      .from('project_phases')
      .select('id, account_id, job_id, name')
      .eq('id', input.phaseId)
      .eq('account_id', input.accountId)
      .maybeSingle();
  ...
  async updateJobTask(input: UpdateJobTaskInput) {
    ...
    const { data: task, error: taskErr } = await this.db
      .from('tasks')
      .select('id, job_id, user_id, title')
      .eq('id', input.taskId)
      .maybeSingle();

    if (taskErr) this.throwErr(taskErr);
    if (!task || task.job_id !== input.jobId) {
